...
def test_remove_root_2_a_typical():
    heap = Heap(2, [1, 2, 3, 7, 8, 9, 4, 5, 12])
    assert heap.list() == [None, 12, 8, 9, 7, 1, 3, 4, 5, 2]
    heap.remove_root()
    assert heap.list() == [None, 9, 8, 4, 7, 1, 3, 2, 5]


def test_remove_root_5_a_typical():
    heap = Heap(5, [1, 2, 3, 7, 8, 9, 4, 5, 10, 12])
    assert heap.list() == [None, 12, 10, 3, 7, 8, 9, 4, 5, 1, 2]
    heap.remove_root()
    assert heap.list() == [None, 10, 5, 3, 7, 8, 9, 4, 2, 1]


def test_remove_root_7_a_typical():
    heap = Heap(7, [1, 2, 3, 7, 15, 6, 20, 18, 19, 8, 9, 4, 5, 10, 12, 31])
    assert heap.list() == [None, 31, 19, 3, 7, 15, 6, 20, 18, 2, 8, 9,
                           4, 5, 10, 12, 1]
    heap.remove_root()
    assert heap.list() == [None, 20, 19, 3, 7, 15, 6, 1, 18, 2, 8, 9,
                           4, 5, 10, 12]